...
int main(int argc, char *argv[]) {
  ...
  for(int i = 0; i < width; i++){
    for(int j = -2; j < height+2; j++){
      ...
      //}
      //unsigned char  buffer_1[4] = {0};
      read_data_from_ACC(SOBELFILTER_READ_ADDR, buffer, 4);
      //std::cout<< int(buffer[0]) << ", " ;
      for(int count = 0; count <= 10000; count ++);
      //std::cout<< int(buffer[0]) << "; ";
      memcpy(data.uc, buffer, 4);
      //auto total = (data).sint;
      if(j>=2){
        *(target_bitmap + bytes_per_pixel * (width * (j-2) + i) + 2) = buffer[0];
        *(target_bitmap + bytes_per_pixel * (width * (j-2) + i) + 1) = buffer[0];
        *(target_bitmap + bytes_per_pixel * (width * (j-2) + i) + 0) = buffer[0];
      }
    }
  }
